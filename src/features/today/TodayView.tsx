...
import { fade, fadeGroup, materialize, transition } from '../../motion/tokens'
...
interface TodayViewProps {
   ...
   */
  step: number
  /** First appearance after the entry ritual: surfaces materialize instead of fading in. */
  materialize?: boolean
}
 ...
 */
export function TodayView({ onStartFocus, step, materialize: materializeOnMount = false }: TodayViewProps) {
  // Fixed for this mount: swapping variants on mounted elements re-applies their hidden state.
  const [materializing] = useState(materializeOnMount)
  const { view } = useDay()
  const item = materializing ? materialize : fade
  ...
  return (
    <m.main
      ...
      variants={materializing ? fadeGroup(0.13, 0.05) : fadeGroup(0.09, 0.05)}
      initial="hidden"
      animate="visible"
    ...
    >
      ...
      <div className="mt-8 grid gap-5 sm:mt-11 lg:mt-14 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-x-16 xl:gap-x-24">
        ...
        <m.aside variants={item} className="mt-6 lg:mt-2">
          <m.div animate={secondary} transition={secondaryTransition}>
